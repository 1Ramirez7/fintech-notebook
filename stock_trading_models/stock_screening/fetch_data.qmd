...
This code does what # cell 73747 but it is closer to the final as it reads stock list, date range and fetches prices for those dates. 
- It makes date_id column and id column. it also duplicates all rows to make sure each ticker and date have their own row. 
...
# code here is to have 5 second break every x tickers to avoid 500 error. 
frames = []
for i, row in enumerate(ticker_lookup.itertuples(), start=1):
    ...
    prices["ticker_id"] = row.ticker_id
    frames.append(prices)

    if i % 120 == 0:
        print(f"Processed {i} tickers... pausing for 5 seconds.")
        time.sleep(5)
# end of code for break to avoid 500 error. 

# ── PARAMETERS ─────────────────────────────────────────────
FILE_PATH  = "sql/fact_close_price.csv"   # existing fact table
START_DATE = "2020-06-01"                 # inclusive
END_DATE   = "2025-06-12"                 # exclusive (yfinance)
# -----------------------------------------------------------
...
ticker_lookup = df_existing[["ticker", "ticker_id"]].drop_duplicates()


# 2. Pull close prices for each ticker
def fetch_prices(ticker):
    ...
    data["calendar_date"] = data["calendar_date"].dt.date
    return data

frames = []
for _, row in ticker_lookup.iterrows():
    prices = fetch_prices(row["ticker"])
    if prices.empty:
        continue
    prices["ticker"]    = row["ticker"]
...
df_new["date_id"] = pd.to_datetime(df_new["calendar_date"]).dt.strftime("%Y%m%d").astype(int)

# 4. Build surrogate id
BASE_DATE  = datetime(2000, 1, 1)
TOTAL_DAYS = (datetime(2027, 12, 31) - BASE_DATE).days + 1  # 10227

df_new["day_count"] = (pd.to_datetime(df_new["calendar_date"]) - BASE_DATE).dt.days + 1
...
df_new = df_new[["id", "ticker_id", "ticker", "date_id", "close_price"]]

# 6. Append to fact table, dropping duplicate ids
df_final = (
    pd.concat([df_existing, df_new], ignore_index=True)
      .drop_duplicates(subset="id")
      .sort_values(["ticker_id", "date_id"])
)
...
- This code pauses every 120 tickers which allows doing all 3000+ tickers at once without hitting the 500 error for to many request. 
- This code gets the close price for data range so it is not yet fitted to do tickers whose price is missing (either did not include when first ran)
- It will not keep duplicated id so close price for stocks should not duplicate. 
- 
...
END_DATE   = "2018-12-31"                 # exclusive (yfinance)
REQUEST_BATCH_SIZE = 120                  # Number of tickers to process before a pause
PAUSE_DURATION_SECONDS = 10               # Duration of the pause in seconds
# -----------------------------------------------------------
...
for i, row in enumerate(ticker_lookup.itertuples(), start=1):
    prices = fetch_prices(row.ticker)
    if not prices.empty: # Only append if data was successfully fetched
        prices["ticker"]    = row.ticker
        prices["ticker_id"] = row.ticker_id
        frames.append(prices)
...
# 5. Select/rename final columns
df_new = df_new[["id", "ticker_id", "ticker", "date_id", "date", "close_price"]]
...
I have this csv with close price data. 

FILE_PATH  = "sql/fact_close_price.csv" 

variables: "id", "ticker_id", "ticker", "date_id", "close_price"

This makes the date_id
df_new["date_id"] = pd.to_datetime(df_new["calendar_date"]).dt.strftime("%Y%m%d").astype(int)
...
- This code will add by "id" so add new id or replace existing ids. 
  - The reason I'm replacing ids is when I add new information, to make sure it is updated, 
  - eventually I just want to updated the change fields and not replace the whole id. 
    - Actually this should be mandatory. 
    - So when joining I should specify what columns should be updated. 
...
https://raw.githubusercontent.com/1Ramirez7/fintech-notebook/refs/heads/main/stock_trading_models/stock_screening/sql/index.csv
