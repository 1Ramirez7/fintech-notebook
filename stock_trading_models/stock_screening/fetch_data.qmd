...
**This cell displays all informationa availbale via yfinance**
```{python}
...
- Data warehousing: least commmon dinominator.
...
# Making fact_industry table

- I will manually make a csv and uploaded to google drive.
- It will have the followig columns
- id_ind, industry_id, industry, date_id, date, volume, close_price, dayhigh, daylow

The following is the logic for the fact_industry table:

- The main code will pull one day of data from the fact_close_price table, and will sum values base on their respective insdustries. 

id_ind will be a composite key of date order and insdustry_id (just like composite for id in fact_close_price).

- indsutry_id and industry will be joined from index_main.csv where ticker_id in fact_close_price.csv is equal to ticker_id in index_main.csv. (after the one day data has been pulled)

- Will obtain volume, close_price, dayhigh, daylow from fact_close_price.csv and they will be a sum of the industry for that day. 

- By now should have industry only values for volume, close_price, dayhigh, daylow.


Problem: I want to do this since 2000. 

I also want to add a column like indsutry total for volume. This will be hard because shares outstanding is reported quaterly and it will be a bit hard to obtain the historical data so this will be over time that I get this data. 


I'm done editing here, the rest of edits I will do in google colab for fact_industry and fact_sector.

```{python}
# this code makes the fact_industry columns giving the fact_industry and columns already exist. if columns dont exist in the fact_industry they will be made. 
# this code has been discontinued and only using the one in google colab. 
# in google colab I mirror this code for the fact_sector. Notes on how I made fact_sector will be on google colab. 
import pandas as pd
from datetime import datetime
...
# Filter by numeric date_id
filtered_df = filtered_df[filtered_df['date_id'] == 20250613]


# Load index.csv from GitHub
index_url = "https://raw.githubusercontent.com/1Ramirez7/fintech-notebook/refs/heads/main/stock_trading_models/stock_screening/sql/index_main.csv"
index_df = pd.read_csv(index_url)

# Select only needed columns
index_df = index_df[['ticker_id', 'industry_id', 'industry']]

# Join on ticker_id
merged_df = filtered_df.merge(index_df, on='ticker_id', how='left')

# Group by industry_id and aggregate multiple columns
grouped_df = (
    merged_df.groupby(['industry_id', 'industry', 'date_id', 'date'], as_index=False)
             .agg({
                 'volume': 'sum',
                 'close_price': 'sum',
                 'dayHigh': 'sum',
...
BASE_DATE = datetime(2000, 1, 1)
TOTAL_DAYS = (datetime(2027, 12, 31) - BASE_DATE).days + 1  # 10227
grouped_df["day_count"] = (pd.to_datetime(grouped_df["date"]) - BASE_DATE).dt.days + 1
grouped_df["id_ind"] = grouped_df.apply(
    lambda r: r["day_count"]
    if r["industry_id"] == 1
    else ((r["industry_id"] - 1) * TOTAL_DAYS + r["day_count"]),
    axis=1,
)

# reading fact_industry.csv to join new data
FILE_PATH = "sql/fact_industry.csv" 
df_existing = pd.read_csv("sql/fact_industry.csv")


# 5. Select/rename final columns
grouped_df = grouped_df[["id_ind", "industry_id", "industry", "date_id", "date", "volume", "close_price", "dayHigh", "dayLow"]]

# 6. Define columns to update in fact_close_price.csv
update_cols = ["close_price", "dayHigh", "dayLow", "volume"] # maybe include date_id & date?

df_updates = grouped_df[["id_ind"] + update_cols].set_index("id_ind")
df_existing.set_index("id_ind", inplace=True)
df_existing.update(df_updates)

new_rows = grouped_df[~grouped_df["id_ind"].isin(df_existing.index)].set_index("id_ind")

df_final = pd.concat([df_existing, new_rows])
# 7. ─────────────────────────────────────────────────────────
df_final.reset_index(inplace=True)
df_final.sort_values(["industry_id", "date_id"], inplace=True)
